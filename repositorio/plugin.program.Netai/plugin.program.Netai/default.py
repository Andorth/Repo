...
def viewBuild(name):
    ...
    for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        addFile('[COLOR lime][B]INSTALAR VERSION BASE[/B][/COLOR]'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
        addFile('[COLOR yellow][B]INSTALAR PARCHE[/B][/COLOR]', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def apkScraper(name=""):
    if name == 'kodi':
        ...
        addFile("Versiones Oficiales SPMC", themeit=THEME1)
...
def apkMenu(name=None, url=None):
    if url == None:
        addDir ('Kodi oficial Apk\'s', 'apkscrape', 'kodi', icon=ICONAPK, themeit=THEME1)
        addDir ('SPMC oficial Apk\'s', 'apkscrape', 'spmc', icon=ICONAPK, themeit=THEME1)
        if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
    if not APKFILE == 'http://':
        if url == None:
...
def maintMenu(view=None):
	...
	addFile('Abrir Menus: %s' % maint.replace('true',on).replace('false',off) ,'togglesetting', 'showmaint', icon=ICONMAINT, themeit=THEME2)
	addDir ('[I]<< Volver al Menu Principal Wizard NETAI[/I]', icon=ICONMAINT, themeit=THEME2)
	setView('files', 'viewType')
...
def writeAdvanced(name, url):
    ...
    if ADVANCEDWORKING == True:
        if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres sobreescribir el actual Buffer con [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR springgreen]Escribir[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
    ...
    if type == 'gui':
        ...
        if yes_pressed:
            ...
            if int(float(percent)) > 0:
                ...
                else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Cerrar[/COLOR] Kodi o [COLOR %s]Recargar el perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar[/COLOR][/B]", nolabel="[B][COLOR springgreen]Cerrar[/COLOR][/B]")
                if todo == 1: wiz.reloadFix()
