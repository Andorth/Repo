#ADVSET Por Chemai Gracias !!

####################################################################
#
# Notbuffer 4 Por Netai, modificacion de Avanced settins, y eliminacion de cosas obsoletas.
# Simplificacion de contenido e interfaz.
# Traduccion y adaptacion por Chemai.
####################################################################
...
def index():
	...
	addFile ('[COLOR gold]Este programa es una modificacion del programa ADVSET, Agradecimientos a su creador Chem@i![/COLOR] ' ,'grupo', icon=ICONSPEED, themeit=THEME3)
	
	addFile ('[COLOR gold][B] NotBuffer 4  [/B][/COLOR] [COLOR ligthblue] Version 1.6 [/COLOR]' ,'grupo', icon=ICONSPEED, themeit=THEME3)
...
def maintMenu(view=None):
	...
	feq        = ['Siempre', 'A diario', '2 Dias', '3 Dias', '5 Dias', 'Semanal']
	
	
	addDir ('[COLOR gold]Ajustar Buffer[/COLOR]',       'maint', 'tweaks', icon=ICONMAINT, themeit=THEME1)
...
def changeFeq():
	feq        = ['Cada inicio', 'Cada dia', 'cada dos dias', 'Cada tres dias', 'cada 5 dias', 'Semanalmente']
	change     = DIALOG.select("[COLOR %s]Con que frecuencia quieres que se ejecute?[/COLOR]" % COLOR2, feq)
	if not change == -1: 
		wiz.setS('autocleanfeq', str(change))
		wiz.LogNotify('[COLOR %s]Auto Limpieza[/COLOR]' % COLOR1, '[COLOR %s]Establecida en %s[/COLOR]' % (COLOR2, feq[change]))
...
def speedMenu():
	xbmc.executebuiltin('Runscript("special://home/addons/plugin.program.NotBuffer4/speedtest.py")')		
