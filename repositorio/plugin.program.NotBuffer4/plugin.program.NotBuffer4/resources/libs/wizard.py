...
def highlightText(msg):
	msg = msg.replace('\n', '[NL]')
	matches = re.compile("-->Python callback/script devuelve el siguiente error<--(.+?)-->Fin del mensaje de error del script Python<--").findall(msg)
	for item in matches:
		string = '-->Python callback/script returned the following error<--%s-->Fin del mensaje de error del script Python<--' % item
...
def getCacheSize():
	...
	if INCLUDEVIDEO == 'true':
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDEBOB == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.palantir', 'cache.db'))
			if INCLUDEPHOENIX == 'true': files.append(os.path.join(ADDONDATA, 'plugin.video.alfa', 'cache.db'))
			if INCLUDESPECTO == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.Realstream', 'cache.db'))
			if INCLUDEGENESIS == 'true': files.append(os.path.join(ADDONDATA, 'plugin.video.cristalazul', 'cache.db'))
			if INCLUDEEXODUS == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.1x2', 'cache.db'))
...
def removeFolder(path):
	log("Borrando Carpeta: %s" % path, xbmc.LOGNOTICE)
	try: shutil.rmtree(path,ignore_errors=True, onerror=None)
	except: return False

def removeFile(path):
	log("Borrando Archivo: %s" % path, xbmc.LOGNOTICE)
	try:    os.remove(path)
	except: return False
			...
			for f in files:
				try: 
					...
					try:
						...
						log("Error Borrando %s" % f, xbmc.LOGERROR)
			for d in dirs:
				total_folds += 1
				try: 
					...
					log("Error Borrando %s" % d, xbmc.LOGERROR)
...
def emptyfolder(folder):
	...
	for root, dirs, files in os.walk(folder, topdown=True):
		...
		if file_count == 0:
			...
			total += 1
			log("Carpetas Vacias : %s" % root, xbmc.LOGNOTICE)
	return total
	...
	if os.path.exists(dep):
		...
		for depends in match:
			if not 'xbmc.python' in depends:
				...
				if not DP == None: 
					DP.update("","Comprobando Dependencias [COLOR yellow]%s[/COLOR] para [COLOR yellow]%s[/COLOR]" % (depends, name),"")
				if os.path.exists(dependspath):
					toggleAddon(name, 'true')
			xbmc.sleep(100)

def toggleAdult():
	do = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Desea [COLOR %s]Habilitar[/COLOR] o [COLOR %s]Desactivar[/COLOR] los addons para Adultos?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR green]Habilitar[/COLOR][/B]", nolabel="[B][COLOR red]Desactivar[/COLOR][/B]")
	...
	if len(found) > 0: 
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Desea ver una lista de addons para Adulto que hay %s?[/COLOR]" % (COLOR2, goto.replace('ing', 'ed')), yeslabel="[B][COLOR green]Ver Lista[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]"):
		...
		forceUpdate(True)
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]No se encontraron Addons para adultos[/COLOR]" % COLOR2)
...
def cleanupBackup():
	...
	list = []; filelist = []
	if len(folder) == 0:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Carpeta de Backup: Vacia[/COLOR]" % (COLOR2))
		return
	...
	selected = DIALOG.select("%s: Seleccione elementos a eliminar de 'MyBuilds'." % ADDONTITLE, list)
	
	if selected == -1:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Cancelado![/COLOR]" % COLOR2)
	elif selected == 0: 
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Le gustaria eliminar todos los elementos de 'My_Builds'?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLOR green]Eliminar[/COLOR][/B]", nolabel="[B][COLOR red]No Cancelar[/COLOR][/B]"):
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres eliminar [COLOR %s]%s[/COLOR] de 'My_Builds'?[/COLOR]" % (COLOR2, COLOR1, list[selected]), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLOR green]Eliminar[/COLOR][/B]", nolabel="[B][COLOR red]No Cancelar[/COLOR][/B]"):
...
def forceUpdate(silent=False):
	...
	if silent == False: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Forzar actualizacion de Addons[/COLOR]' % COLOR2)

def convertSpecial(url, over=False):
	total = fileCount(url); start = 0
	DP.create(ADDONTITLE, "[COLOR %s]Cambiando las rutas a especiales" % COLOR2, "", "Espere[/COLOR]")
	for root, dirs, files in os.walk(url):
		for file in files:
			start += 1
			perc = int(percentage(start, total))
			if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
				DP.update(perc, "[COLOR %s]Scanning: [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, root.replace(HOME, '')), "[COLOR %s]%s[/COLOR]" % (COLOR1, file), "Espere[/COLOR]")
				...
				f.close()
	DP.close()
	log("[Cambiando las rutas] Completado", xbmc.LOGNOTICE)
	if over == False: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s] Cambiar las rutas: Completado![/COLOR]" % COLOR2)
...
def clearCrash():  
	...
	for file in glob.glob(os.path.join(LOG, '*crashlog*.*')):
		files.append(file)
	if len(files) > 0:
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar los Logs de Bloqueo?' % COLOR2, '[COLOR %s]%s[/COLOR] Logs Encontardos[/COLOR]' % (COLOR1, len(files)), yeslabel="[B][COLOR green]Borrar Logs[/COLOR][/B]", nolabel="[B][COLOR red]Conservar Logs[/COLOR][/B]"):
			for f in files:
				os.remove(f)
			LogNotify('[COLOR %s]Borrar Logs de Bloqueo[/COLOR]' % COLOR1, '[COLOR %s]%s Logs Borrados[/COLOR]' % (COLOR2, len(files)))
		else: LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Logs Cancelado[/COLOR]' % COLOR2)
	else: LogNotify('[COLOR %s]Borrar Logs de Bloqueo[/COLOR]' % COLOR1, '[COLOR %s]No hay Logs de Bloqueo[/COLOR]' % COLOR2)

def hidePassword():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]ocultar[/COLOR] las passwords de addons en la configuracion?[/COLOR]" % COLOR2, yeslabel="[B][COLOR green]OCULTAR[/COLOR][/B]", nolabel="[B][COLOR red]No Cancelar[/COLOR][/B]"):
		count = 0
					...
					if 'pass' in line:
						if not 'option="hidden"' in line:
							try:
								...
								count += 1
								log("[Ocultar Passwords] encontrados %s on %s" % (sett.replace(HOME, ''), line), xbmc.LOGDEBUG)
							except:
								pass
...
def unhidePassword():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]mostrar[/COLOR] las passwords de addons en la configuracion?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR green]MOSTRAR[/COLOR][/B]", nolabel="[B][COLOR red]No Cancelar[/COLOR][/B]"):
		count = 0
		for folder in glob.glob(os.path.join(ADDONS, '*/')):
			sett = os.path.join(folder, 'resources', 'settings.xml')
			if os.path.exists(sett):
					...
					if 'pass' in line:
						if 'option="hidden"' in line:
							try:
								...
								f.replace(line, change)
								count += 1
								log("[Mostrar] encontradas %s on %s" % (sett.replace(HOME, ''), line), xbmc.LOGDEBUG)
							except:
								pass
				f2 = open(sett, mode='w'); f2.write(f); f2.close()
		LogNotify("[COLOR %s]Mostrar[/COLOR]" % COLOR1, "[COLOR %s]%s cambiadas[/COLOR]" % (COLOR2, count))
		log("[Mostrar] %s cambiadas" % count, xbmc.LOGNOTICE)
	else: log("[Mostrar] Cancelado", xbmc.LOGNOTICE)
...
def convertText():
	...
	DP.create(ADDONTITLE,'[COLOR %s][B]Convirtiendo Texto:[/B][/COLOR]' % (COLOR2),'', 'Espere')
	
	if not BUILDFILE == 'http://':
		filename = os.path.join(TEXTFILES, 'builds.txt')
		writing = '';x = 0
		a = openURL(BUILDFILE).replace('\n','').replace('\r','').replace('\t','')
		DP.update(0,'[COLOR %s][B]Convirtiendo Texto:[/B][/COLOR] [COLOR %s]Builds.txt[/COLOR]' % (COLOR2, COLOR1),'', 'Espere')
	...
	if not YOUTUBEFILE == 'http://':
		...
		DP.update(0,'[COLOR %s][B]Convirtiendo Texto:[/B][/COLOR] [COLOR %s]YouTube.txt[/COLOR]' % (COLOR2, COLOR1), '', 'Espere')
	...
	if not ADVANCEDFILE == 'http://':
		...
		DP.update(0,'[COLOR %s][B]Convirtiendo Texto:[/B][/COLOR] [COLOR %s]AdvancedSettings.txt[/COLOR]' % (COLOR2, COLOR1), '', 'Espere')
		match = re.compile('name="(.+?)".+?rl="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(a)
		for name, url, icon, fanart, description in match:
			x += 1
			DP.update(int(percentage(x, len(match))), '', "[COLOR %s]%s[/COLOR]" % (COLOR1, name))
	...
	DP.close()
	DIALOG.ok(ADDONTITLE, '[COLOR %s]Tus archivos de texto han sido convertidos a 0.1.7 y en la carpeta [COLOR %s]/addon_data/%s/[/COLOR] estan guardados[/COLOR]' % (COLOR2, COLOR1, ADDON_ID))
...
def asciiCheck(use=None, over=False):
	if use == None:
		...
		if over == True:
			...
			yes = DIALOG.yesno(ADDONTITLE,'[COLOR %s]Quieres [COLOR %s]BORRAR[/COLOR] todods los nombres de archivo con caracteres especiales o prefieres solo [COLOR %s]ESCANEAR[/COLOR] y ver los resultados en el log?[/COLOR]' % (COLOR2, COLOR1, COLOR1), yeslabel='[B][COLOR green]BORRAR[/COLOR][/B]', nolabel='[B][COLOR red]ESCANEAR[/COLOR][/B]')
	else: 
		source = use
		yes = 1

	if source == "":
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]ASCII Check: Cancelado[/COLOR]" % COLOR2)
		return
	...
	for base, dirs, files in os.walk(source):
		...
		for file in files:
			...
			DP.update(prog2,"[COLOR %s]Comprobando archivos ASCII" % COLOR2,'[COLOR %s]%s[/COLOR]' % (COLOR1, d), 'Espere[/COLOR]')
			try:
				file.encode('ascii')
				...
				if yes:
					try: 
						...
						afiles.write('\n')
						f1 += 1
						log("[ASCII Check] Borrados: %s " % badfile, xbmc.LOGERROR)
					except:
						for chunk in chunks(badfile, 75):
							afails.write(chunk+'\n')
						afails.write('\n')
						f2 += 1
						log("[ASCII Check] Fallos: %s " % badfile, xbmc.LOGERROR)
				else:
					for chunk in chunks(badfile, 75):
						afiles.write(chunk+'\n')
					afiles.write('\n')
					f1 += 1
					log("[ASCII Check] Encontrado: %s " % badfile, xbmc.LOGERROR)
				pass
	...
	if total > 0:
		...
		if yes:
			if use:
				LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]ASCII Check: %s Borrado / %s Fallos.[/COLOR]" % (COLOR2, f1, f2))
			else:
				TextBox(ADDONTITLE, "[COLOR yellow][B]%s Archivos Borrados:[/B][/COLOR]\n %s\n\n[COLOR yellow][B]%s Archivos con Fallo:[B][/COLOR]\n %s" % (f1, msg, f2, msg2))
		else: 
			TextBox(ADDONTITLE, "[COLOR yellow][B]%s Encontrados:[/B][/COLOR]\n %s" % (f1, msg))
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s] Ningun cambio.[/COLOR]" % COLOR2)
...
def backUpOptions(type, name=""):
	...
	try:
		...
		DIALOG.ok(ADDONTITLE, "[COLOR %s]Error al crear Directorio de BackUp:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
		return
	if type == "build":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Deseas hacer un BackUp de la compilacion actual?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancelar[/COLOR][/B]", yeslabel="[B][COLOR green]HACER BackUp[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Nombre del  archivo %s zip" % type)
			...
			for_progress  = 0
			ITEM          = []
			if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Desea incluir su carpeta addon_data?" % COLOR2, 'Esta contiene [COLOR %s]TODOS[/COLOR] los ajustes de los addons incluyendo contrasenas pero tambien informacion importante, como atajos de teclado. Mejor [COLOR %s]ELIMINAR MANUALMENTE[/COLOR] las carpetas addon_data que no son necesarias.' % (COLOR1, COLOR1), '[COLOR %s]%s[/COLOR] addon_data sera ignorado[/COLOR]' % (COLOR1, ADDON_ID), yeslabel='[B][COLOR green]Incluir DATA[/COLOR][/B]',nolabel='[B][COLOR red]NO INCLUIR[/COLOR][/B]'):
			...
			try:
				...
				try:
					...
					log("Imposible crear %s.zip" % name, xbmc.LOGERROR)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]No se puede escribir en el directorio de BackUp actual, Quieres cambiar la ubicacion?[/COLOR]" % COLOR2, yeslabel="[B][COLOR green]CAMBIAR[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
						openS()
						return
					else:
						return
			DP.create("[COLOR %s]%s[/COLOR][COLOR %s]: Creando Zip[/COLOR]" % (COLOR1, ADDONTITLE,COLOR2), "[COLOR %s]Creando BackUp zip" % COLOR2, "", "Espere...[/COLOR]")
			for base, dirs, files in os.walk(HOME):
				dirs[:] = [d for d in dirs if d not in exclude_dirs]
				files[:] = [f for f in files if f not in exclude_files]
				for file in files:
				...
				for file in files:
					try:
						...
						try:
							...
							log("[Back Up] Type = '%s': Imposible hacer BackUp %s" % (type, file), xbmc.LOGNOTICE)
							log("%s / %s" % (Exception, e))
					except Exception, e:
						log("[Back Up] Type = '%s': Imposible hacer BackUp %s" % (type, file), xbmc.LOGNOTICE)
						log("Error haciendo BackUp : %s" % str(e), xbmc.LOGNOTICE)
			zipf.close()
			xbmc.sleep(500)
			DP.update(100, "Crear %s_guisettings.zip" % name, "", "")
			...
			DP.close()
			DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR] [COLOR %s]BackUp realizado:[/COLOR]" % (COLOR1, name, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, zipname))
	elif type == "guifix":
		if name == "":
			guiname = getKeyboard("","Nombre para el %s zip" % type)
		...
		if os.path.exists(GUISETTINGS):
			try:
				...
				try:
					...
					log("Imposible crear %s_guisettings.zip" % guiname, xbmc.LOGERROR)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]No se puede escribir en el directorio de BackUp actual, Quieres cambiar la ubicacion?[/COLOR]" % COLOR2, yeslabel="[B][COLOR green]CAMBIAR[/COLOR][/B]", nolabel="[B][COLOR red]CANCELAR[/COLOR][/B]"):
						openS()
						return
					else:
			...
			try:
				...
				for fold in match:
					...
					if not fd in ['skin.confluence', 'skin.re-touch', 'skin.estuary', 'skin.estouchy']:
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres anadir la siguiente carpeta del skin al archivo GuiFix Zip?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, fd), yeslabel="[B][COLOR green]SUMAR skin[/COLOR][/B]", nolabel="[B][COLOR red]OMITIR Skin[/COLOR][/B]"):
		...
		if name == "":
			DIALOG.ok(ADDONTITLE, "[COLOR %s]GuiFix BackUp Realizado:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, guizipname))
	elif type == "theme":
...
def restoreLocal(type):
	...
	try:
		...
		DIALOG.ok(ADDONTITLE, "[COLOR %s]Error creando Directorio del BackUp:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
		return
	file = DIALOG.browse(1, '[COLOR %s]Seleccione el archivo a Restaurar[/COLOR]' % COLOR2, 'files', '.zip', False, False, mybuilds)
	log("[RESTORE BACKUP %s] File: %s " % (type.upper(), file), xbmc.LOGNOTICE)
	if file == "" or not file.endswith('.zip'):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Restaurar: Cancelado[/COLOR]" % COLOR2)
		return
	DP.create(ADDONTITLE,'[COLOR %s]Instalando Backup' % COLOR2,'', 'Espere[/COLOR]')
	...
	try:
		...
		DP.update(0, '[COLOR %s]Imposible leer zip de la ubicacion actual.' % COLOR2, 'Copiando archivo a packages')
		pack = os.path.join('special://home', 'addons', 'packages', fn)
		xbmcvfs.copy(file, pack)
		file = xbmc.translatePath(pack)
		DP.update(0, '', 'Copying file to packages: Realizado')
		zipfile.ZipFile(file, 'r')
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar Salida[/COLOR] de kodi o [COLOR %s]Recargar Perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar Perfil[/COLOR][/B]", nolabel="[B][COLOR green]Forzar Salida[/COLOR][/B]")
	if todo == 1: reloadFix()
	else: killxbmc(True)
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Borrar Packages" % COLOR2, "[COLOR %s]%s[/COLOR] encontrados / [COLOR %s]%s[/COLOR] in size." % (COLOR1, str(file_count), COLOR1, size), "Quires borrarlos?", nolabel='[COLOR red]No Borrar[/COLOR]',yeslabel='[COLOR green]Borrar Packages[/COLOR]')
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Borrar Packages: Realizado![/COLOR]' % COLOR2)
				else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Borrar Packages: No encontrados![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Borrar Packages: Error![/COLOR]' % COLOR2)
			log("Borrar Packages Error: %s" % str(e), xbmc.LOGERROR)
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Borrar Packages: No encontrados![/COLOR]' % COLOR2)
				...
				if lastedit <= start:
					if os.path.isfile(file):
						...
						try:
							...
							log("Fallo al borrar %s: %s" % (file, str(e), xbmc.LOGERROR))
			if file_count > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Packages: realizado: %s[/COLOR]' % (COLOR2, convertSize(cleanupsize)))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Packages: No encontrados![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Packages: Error![/COLOR]' % COLOR2)
			log("Borrar Packages Error: %s" % str(e), xbmc.LOGERROR)
...
def clearCache(over=None):
	PROFILEADDONDATA = os.path.join(PROFILE,'addon_data')
	dbfiles   = [
		(os.path.join(ADDONDATA, 'plugin.video.Realstream', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.alfa', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.cristalazul', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.palantir', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.cine', 'cache.db')),
		(os.path.join(DATABASE,  'onechannelcache.db')),
		(os.path.join(DATABASE,  'saltscache.db')),
		(os.path.join(DATABASE,  'saltshd.lite.db'))]
	...
	cachelist = [
		...
		(os.path.join(ADDONDATA,'plugin.video.itv','Images')),
		(os.path.join(PROFILEADDONDATA,'script.module.simple.downloader')),
		(os.path.join(PROFILEADDONDATA,'plugin.video.Realstream','Images'))]
				...
				if file_count > 0:
					for f in files:
						if not f in LOGFILES:
							try:
								os.unlink(os.path.join(root, f))
								log("[Borrado] %s" % os.path.join(root, f), xbmc.LOGNOTICE)
								delfiles += 1
							except:
								pass
					...
					for d in dirs:
						try:
							shutil.rmtree(os.path.join(root, d))
							delfiles += 1
							log("[Realizado] borrados %s archivos de %s" % (str(file_count), os.path.join(item,d)), xbmc.LOGNOTICE)
						except:
							log("[Fallo] borrar cache en: %s" % os.path.join(item,d), xbmc.LOGNOTICE)
		else:
			for root, dirs, files in os.walk(item):
				dirs[:] = [d for d in dirs if d not in excludes]
				for d in dirs:
					if not str(d.lower()).find('cache') == -1:
						try:
							shutil.rmtree(os.path.join(root, d))
							delfiles += 1
							log("[Realizado] Borrado %s " % os.path.join(root,d), xbmc.LOGNOTICE)
						except:
							log("[Fallo] borrar cache en: %s" % os.path.join(item,d), xbmc.LOGNOTICE)
	if INCLUDEVIDEO == 'true' and over == None:
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDEBOB == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.palantir', 'cache.db'))
			if INCLUDEPHOENIX == 'true': files.append(os.path.join(ADDONDATA, 'plugin.video.cine', 'cache.db'))
			if INCLUDESPECTO == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.Realstream', 'cache.db'))
			...
			for item in files:
				if os.path.exists(item):
					...
					if 'Database' in item:
						...
						textexe.close()
		else: log("Borrar Cache: Borrado de Cache de Video sin activar", xbmc.LOGNOTICE)
	LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Cache: Borrado %s Archivos[/COLOR]' % (COLOR2, delfiles))

def checkSources():
	if not os.path.exists(SOURCES):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]No hay archivo de Fuentes![/COLOR]" % COLOR2)
		return False
	x      = 0
	bad    = []
	remove = []
	...
	if len(match) > 0:
		...
		log("Fuentes erroneas: %s" % len(bad), xbmc.LOGNOTICE)
		if len(bad) > 0:
			choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] Fuentes(s) encontradas rotas" % (COLOR1, len(bad), COLOR2),"Quieres removerlas todas o seleccionar una a una?[/COLOR]", yeslabel="[B][COLOR green]TODAS[/COLOR][/B]", nolabel="[B][COLOR red]Elegir cual borrar[/COLOR][/B]")
			if choice == 1:
				remove = bad
			else:
				for name, path, sharing, working in bad: 
					log("%s fuentes: %s, %s" % (name, path, working), xbmc.LOGNOTICE)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] catalogada como no funcional" % (COLOR1, name, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLOR green]Borrar Fuente[/COLOR][/B]", nolabel="[B][COLOR red]Mantener[/COLOR][/B]"):
						remove.append([name, path, sharing, working])
						log("Borrando Fuente %s" % name, xbmc.LOGNOTICE)
					else: log("Fuente %s no fue borrada" % name, xbmc.LOGNOTICE)
			if len(remove) > 0:
				...
				removed = len(remove)
				DIALOG.ok(ADDONTITLE, "[COLOR %s]La verificacion de Fuentes a finalizado" % COLOR2, "Activas: [COLOR %s]%s[/COLOR] | Mantenidas: [COLOR %s]%s[/COLOR] | Borradas: [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR2, COLOR1, alive, COLOR1, kept, COLOR1, removed))
			else: log("No hay malas erroneas para eliminar.", xbmc.LOGNOTICE)
		else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Todas las fuentes estan operativas[/COLOR]" % COLOR2)
	else: log("No se encontraron Fuentes", xbmc.LOGNOTICE)
...
def checkRepos():
	...
	if len(repolist) == 0:
		DP.close()
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]No se encontraron Repositorios![/COLOR]" % COLOR2)
		return
	...
	while start < sleeptime:
		...
		DP.update(perc, '', '[COLOR %s]Comprobandos: [/COLOR][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, repolist[start-1].replace(ADDONS, '')[1:]))
		xbmc.sleep(1000)
	if DP.iscanceled(): 
		DP.close()
	...
	for item in fails:
		log("Repositoros Mal: %s " % item, xbmc.LOGNOTICE)
		brokenrepo = item.replace('[','').replace(']','').replace(' ','').replace('/','').replace('\\','')
		if not brokenrepo in badrepos:
			badrepos.append(brokenrepo)
	if len(badrepos) > 0:
		msg  = "[COLOR %s]A continuacion hay una lista de repositorios inciertos.  Esto no significa que esten abandonados, a veces los host bajan por un corto periodo de tiempo. Haga una serie de escaneos de su lista de repositorios antes de eliminarlo, para asegurarse de que este roto.[/COLOR][CR][CR][COLOR %s]" % (COLOR2, COLOR1)
		msg += '[CR]'.join(badrepos)
		msg += '[/COLOR]'
		TextBox("%s: Errores en Repositorios" % ADDONTITLE, msg)
	else: 
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Todos lod Repositorios Funcionan![/COLOR]" % COLOR2)


def killxbmc(over=None):
	if over: choice = 1
	else: choice = DIALOG.yesno('Forzar el cierre de Kodi', '[COLOR %s]Estas a punto de cerrar Kodi' % COLOR2, 'Quieres continuar?[/COLOR]', nolabel='[B][COLOR red] No Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Forzar cierre[/COLOR][/B]')
	if choice == 1:
		log("Forzar cierre: Platform[%s]" % str(platform()), xbmc.LOGNOTICE)
		os._exit(1)
...
def reloadFix(default=None):
	DIALOG.ok(ADDONTITLE, "[COLOR %s]ADVERTENCIA:A veces provoca cuelgues en Kodi. Mientras Kodi recarga el perfil, No presione ningun boton![/COLOR]" % COLOR2)
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	if default == None:
		lookandFeelData('save')
	redoThumbs()
...
def swapSkins(goto):
	...
	if xbmc.getCondVisibility("Window.isVisible(yesnodialog)"):
		ebi('SendClick(11)')
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Fresh Install: Se agoto el timpo par cambiar skin![/COLOR]' % COLOR2); return False
	xbmc.sleep(500)
			...
			try:
				...
				try:
					...
					try:
						...
						if len(aid) == 0: log("Imposible activar: %s(No se puede determinar Addon ID)" % fold, xbmc.LOGERROR)
						else: log("Imposible activar: %s" % folder, xbmc.LOGERROR)
	if len(disabledAddons) > 0:
		x = 0
		DP.create(ADDONTITLE,'[COLOR %s]Habilitando Addons Desactivados' % COLOR2,'', 'Espere[/COLOR]')
		for item in disabledAddons:
			x += 1
			prog = int(percentage(x, len(disabledAddons)))
			DP.update(prog, "", "Activados: [COLOR %s]%s[/COLOR]" % (COLOR1, item))
			addonDatabase(item, 1)
			if DP.iscanceled(): break
		if DP.iscanceled(): 
			DP.close()
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Activacion Cancelada![/COLOR]" % COLOR2)
			sys.exit()
		DP.close()
	forceUpdate()
	ebi("ReloadSkin()")
...
def purgeDb(name):
	...
	LogNotify("[COLOR %s]Borrar Database[/COLOR]" % COLOR1, "[COLOR %s]%s Completada[/COLOR]" % (COLOR2, show[len(show)-1]))
...
def oldThumbs():
	...
	if len(images) > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Thumbs: %s Files / %s MB[/COLOR]!' % (COLOR2, str(len(images)), removed))
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Thumbs: Ninguno![/COLOR]' % COLOR2)
